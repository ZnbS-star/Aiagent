...
**4. 格式与内容规范 (Formatting and Content Rules):**
- 对于每种题型，请先写出大标题（例如：“一、选择题”）。
- 请为每个题目进行全局连续编号（例如“题目1”、“题目2”...）。
- ## 新增的关键指令 ##
  **严禁在每个题目下方直接给出答案。试卷的题目部分（在“参考答案与解析”部分之前）必须保持纯净，只包含题干和选项。**

""")
    
    # 动态添加编程题的格式要求 (这部分逻辑保持不变)
...
- **如果要求包含“编程题”(Programming Question)，请务必遵循以下结构：**
    ...
    # 动态添加答案部分的规则，并强化其唯一性
    answer_rules_part = """
- 在试卷的**最后**，请提供一个清晰的“**参考答案与解析**”部分，**所有题目的答案都必须且只能在这里统一列出**。"""
    ...
    human_message_parts.append(answer_rules_part)
